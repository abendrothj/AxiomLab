...
# ── Vessel-state HTTP endpoint ────────────────────────────────────────────────

class _VesselStateHandler(http.server.BaseHTTPRequestHandler):
    """Serves GET /vessel_state as JSON — localhost-only, read-only, no auth needed."""

    registry = None  # set by start_state_server() before the thread starts

    def do_GET(self) -> None:
        if self.path != "/vessel_state":
            self.send_response(404)
            self.end_headers()
            return
        try:
            data = self.__class__.registry.all_volumes() if self.__class__.registry else {}
    ...
    def log_message(self, *_args) -> None:  # suppress per-request access log spam
        pass


def start_state_server(registry, ip: str, state_port: int) -> None:
    """Start a daemon HTTP thread serving vessel volumes on ip:state_port."""
    _VesselStateHandler.registry = registry
    httpd = http.server.HTTPServer((ip, state_port), _VesselStateHandler)
    t = threading.Thread(target=httpd.serve_forever, daemon=True, name="vessel-state-http")
    t.start()
    logger.info(f"Vessel-state HTTP endpoint: http://{ip}:{state_port}/vessel_state")


def main(
    ip_address: str = Option(_DEFAULT_IP, "-a", "--ip-address", help="The IP address"),
    port: int = Option(50052, "-p", "--port", help="The port"),
    state_port: int = Option(0, "--state-port", help="HTTP vessel-state port (0 = port+1)"),
    ...
    # start vessel-state HTTP endpoint (read-only, localhost-only)
    resolved_state_port = state_port if state_port > 0 else port + 1
    start_state_server(server.vessel_registry, ip_address, resolved_state_port)
