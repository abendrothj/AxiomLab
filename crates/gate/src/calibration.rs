...
/// Append a calibration record to the chain. `valid_until` is required.
...
mod tests {
    ...
    #[test]
    fn record_then_read_latest() {
        let dir = tempfile::tempdir().unwrap();
        let chain = Chain::open(dir.path().join("a.jsonl"));
        let s = LocalSigner::generate();
        let future = now() + 3600;
        record_calibration(&chain, &s, "spectrophotometer", future, 0.99).unwrap();
