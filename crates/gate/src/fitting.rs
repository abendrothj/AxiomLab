//! Curve fitting and model selection for `analyze_series`.
//!
//! Ported from the legacy `scientific_compute` crate (OLS, Hill, Michaelis-Menten
//! via Levenberg-Marquardt, AIC model selection). Pure math, no I/O.
//!
//! Kept as a complete primitive set; not every helper is wired into
//! `analyze_series` yet, hence the module-level dead-code allowance.
#![allow(dead_code)]
...
mod tests {
    ...
    fn hill_fit_recovers_parameters() {
        ...
        let y: Vec<f64> = x.iter().map(|&xi| { let xn = xi.powi(2); xn / (100.0 + xn) }).collect();
        let fit = hill_equation_fit(&x, &y).unwrap();
        assert!((fit.ec50 - 10.0).abs() < 0.1, "EC50={:.4}", fit.ec50);
    }
    ...
    fn michaelis_menten_fit_recovers_vmax_km() {
        ...
        assert!((fit.v_max - 100.0).abs() < 0.01);
        assert!((fit.km - 5.0).abs() < 0.01);
    }
