//! The proof manifest — what CI verified, and which artifacts each action needs.
...
impl ProofManifest {
    pub fn artifact(&self, id: &str) -> Option<&ProofArtifact> {
        self.artifacts.iter().find(|a| a.id == id)
    }
    pub fn policy_for(&self, action: &str) -> Option<&ActionPolicy> {
        self.actions.iter().find(|p| p.action == action)
    }
}
